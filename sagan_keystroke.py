"""
Tabular Self-Attention Generative Adversarial Network (Tab-SAGAN)
------------------------------------------------------------------
This script implements a Self-Attention GAN for synthesizing high-fidelity
tabular data, specifically for USB keystroke injection datasets. It combines
the stability of the WGAN-GP framework with the power of self-attention
to model complex inter-feature correlations.

The pipeline includes:
1. Data loading and preprocessing tailored for the dataset.
2. PyTorch implementation of the Generator and Discriminator with Self-Attention.
3. A robust WGAN-GP training loop with hardware acceleration support.
4. A comprehensive evaluation suite:
    - Statistical Fidelity (Kolmogorov-Smirnov Test)
    - Machine Learning Utility (Train-on-Synthetic, Test-on-Real)
    - Visual Analysis (Distributions, Correlations, PCA)
"""
...
class Config:
    ...
    DATA_FILE = 'smartattackdata_original_foldcauchy.csv'

    OUTPUT_DIR = 'sagan_results'
...
class DataHandler:
    ...
    def get_training_dataloader(self) -> DataLoader:
        ...
        print(f"Scaler fitted and saved to {self.cfg.SCALER_PATH}")
...
class SelfAttention(nn.Module):
    """Self-attention layer for 1D tabular data, inspired by SAGAN."""

    def __init__(self, in_dim: int):
        super(SelfAttention, self).__init__()
        self.query_conv = nn.Linear(in_features=in_dim, out_features=in_dim // 8)
        self.key_conv = nn.Linear(in_features=in_dim, out_features=in_dim // 8)
        self.value_conv = nn.Linear(in_features=in_dim, out_features=in_dim)
        self.gamma = nn.Parameter(torch.zeros(1))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        batch_size, C = x.size()
        query = self.query_conv(x).view(batch_size, -1, C // 8)
        key = self.key_conv(x).view(batch_size, -1, C // 8)
        value = self.value_conv(x).view(batch_size, -1, C)

        proj_query = query.permute(0, 2, 1)
        energy = torch.bmm(key, proj_query)
...
class Discriminator(nn.Module):
    """Discriminator (Critic) model for Tab-SAGAN."""
...
class Trainer:
    ...
    def __init__(self, cfg: Config):
        ...
        self.optimizer_G = torch.optim.Adam(self.generator.parameters(), lr=cfg.LEARNING_RATE,
                                            betas=(cfg.BETA_1, cfg.BETA_2))
        self.optimizer_D = torch.optim.Adam(self.discriminator.parameters(), lr=cfg.LEARNING_RATE,
                                            betas=(cfg.BETA_1, cfg.BETA_2))
    ...
    def train(self):
        ...
        for epoch in range(self.cfg.EPOCHS):
            ...
            for i, (real_data_batch,) in progress_bar:
                ...
                z = torch.randn(self.cfg.BATCH_SIZE, self.cfg.LATENT_DIM, device=self.device)
    ...
    def generate_synthetic_data(self, num_samples: int) -> pd.DataFrame:
        ...
        z = torch.randn(num_samples, self.cfg.LATENT_DIM, device=self.device)
        with torch.no_grad():
        ...
        print(f"Generated {num_samples} samples and saved to {self.cfg.SYNTHETIC_DATA_PATH}")
        return synthetic_df
...
class Evaluator:
    ...
    def evaluate_ml_utility(self, real_full: pd.DataFrame, synthetic_benign: pd.DataFrame):
        """Performs Train-on-Synthetic, Test-on-Real evaluation."""
        print("\n2. Evaluating Machine Learning Utility (TSTR)...")
        real_attack = real_full[real_full[self.cfg.LABEL_COL] == self.cfg.ATTACK_CLASS][self.cfg.FEATURE_COLUMNS]

        X_synth_train = pd.concat([synthetic_benign, real_attack], ignore_index=True)
        y_synth_train = np.concatenate([np.zeros(len(synthetic_benign)), np.ones(len(real_attack))])

        X_real = real_full[self.cfg.FEATURE_COLUMNS]
        y_real = (real_full[self.cfg.LABEL_COL] == self.cfg.ATTACK_CLASS).astype(int)

        _, X_real_test, _, y_real_test = train_test_split(X_real, y_real, test_size=0.3, random_state=42,
                                                          stratify=y_real)
        ...
        report_text = f"""
        Machine Learning Utility Report (Train-Synthetic, Test-on-Real)
        ================================================================
        Model: LGBMClassifier

        Methodology:
        - A classifier was trained on a dataset composed of synthetic benign
          data (from Tab-SAGAN) and real attack data.
        - The trained model was then evaluated on a held-out test set
          of real data (both benign and attack).
    ...
    def perform_visual_diagnostics(self, real_benign: pd.DataFrame, synthetic_benign: pd.DataFrame):
        ...
        for i, col in enumerate(self.cfg.FEATURE_COLUMNS):
            sns.kdeplot(real_benign[col], ax=axes[i], label='Real', fill=True, color='blue', alpha=0.5)
            sns.kdeplot(synthetic_benign[col], ax=axes[i], label='Synthetic', fill=True, color='orange', alpha=0.5)
        ...
        real_pca = pca.fit_transform(real_benign)
        synthetic_pca = pca.transform(synthetic_benign)
